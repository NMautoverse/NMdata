...
```{r,aplot-plot,echo=FALSE,eval=TRUE,out.width="105%"}
...
::: {.column width="55%"}
\normalsize
```{r,comparemodels-plot,echo=F,message=FALSE,warning=FALSE}
...
* `NMdata` includes `r Ntests` "unit tests" where results of function calls
...
```{r,load-pmxtricks,include=FALSE}
## load_all("c:/Users/delff/working_copies/pmxtricks/")
load_all("c:/Users/Philip Delff/wdirs/pmxtricks/")
```
...
```{r,install-pmxtricks,eval=FALSE}
library(remotes)
...
\footnotesize
```{r,ggIndProfs,message=FALSE,warning=FALSE,out.height="75%"}
