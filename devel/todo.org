...
*** addTAPD
**** TODO [#A] Check if data contains doses
addTAPD only makes sense if doses are present. For subjects without
doses, all generated doses should be NA?
...
fail.
*** NMcheckData
**** TODO [#A] NMcheckData does not find NA in covs
NMcheckData(dt.sim,col.usubjid="USUBJID",col.row="ROWCPT",covs="WTBL",cols.num="PDOSAMT")
