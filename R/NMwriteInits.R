...
######### Limitation: lower, init, and upper must be on same line
    ...
    inits.w <- dcast(
        pars.l[model+type.elem%in%c("lower","init","upper","FIX")]
...
### todo check ext object
    ...
    lines.new <- lapply(split(inits.w,by="model.ext"),function(dat){
    ...
    if(!is.null(newfile)){
        if(length(lines.new)>1){
            stop("canot write files when number of resulting lines>1.")
        }
